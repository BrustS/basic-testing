...
describe('simpleCalculator tests', () => {
  ...
  test('should subtract two numbers', () => {
    expect(simpleCalculator({ a: 21, b: 3, action: Action.Subtract })).toBe(18);
  });
  ...
  test('should return null for invalid action', () => {
    expect(simpleCalculator({ a: 2, b: 3, action: 34 })).toBeNull();
  });

  test('should return null for invalid arguments', () => {
    expect(
      simpleCalculator({ a: 2, b: 'hello', action: Action.Add }),
    ).toBeNull();
  });
});
